...
- **Markey's Hope-rail speaking-meter real-amplitude gain — RESOLVED 2026-09-06.** Was a flat `getOutputVolume() * 18` (docs previously said `× 11` — that was stale; the live code was `× 18`) copied verbatim from PTT's mic-RMS curve. Kevin's live-call feedback confirmed the predicted symptom (constantly large, no headroom/dynamics). Root cause confirmed by reading the pinned `@elevenlabs/client@0.1.7` dist source directly (no browser/mic tool available to this agent to run a live call itself): `getOutputVolume()` is `calculateVolume(getOutputByteFrequencyData())` — an `AnalyserNode.getByteFrequencyData()` average across all ~1024 default-fftSize bins (WebAudio defaults: fftSize 2048, smoothingTimeConstant 0.8, minDecibels -100, maxDecibels -30), i.e. a smoothed dB-mapped whole-spectrum power average — structurally not the same signal domain as PTT's time-domain mic RMS, which is why a constant tuned for the latter pinned near the ceiling on the former. **Design note:** the first-cut fix (a peak-follower normalizing each sample against its own recent ceiling) went through 3 Codex TP2 rounds that each found it still pinned near 1 or pumped on quiet passages under one attack/decay tuning or another — a structural flaw of that whole approach, not a tunable constant. Final shipped design drops peak-normalization for an AGC/compressor-style reference tracker instead (`feedReal()`/`resetRealPeak()` in `HOPE_WAVE`, branch `markey-hopewave-gain-tune`): a slowly-lagging reference (`REAL_REF_TARGET=0.62`, `REAL_REF_TAU_MS=550`) sets the gain, so a sample louder/quieter than the recent ~500ms average reads genuinely bigger/smaller rather than always re-normalizing near its own ceiling; true pauses (rms≈0) still read near-zero regardless of gain. A related integration bug (a stale `real > 0.004` threshold routing valid-but-quiet real readings into the synthetic fallback instead of the real AGC path) was also fixed. Synth/idle fallback paths themselves untouched — only used when no analyser exists at all. Codex TP2 round 5 = approved. **Still open:** this is a mechanism-level fix with numerically-verified AGC behavior, not a live-amplitude-verified one — Kevin's own quick live listen (same harness he used to report the original bug) is the remaining confirmation step before fully closing, in particular whether `REAL_REF_TARGET`/`REAL_REF_TAU_MS` need adjustment.
...
- D. RESOLVED 2026-09-06 — see the resolved note above (AGC/compressor-style reference-tracking gain shipped on `markey-hopewave-gain-tune`, after 3 Codex rounds ruled out a peak-follower as structurally flawed); Kevin's own live listen is the remaining confirmation step.
