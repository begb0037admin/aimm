...
| **Full-page Mixio-violet redesign** | **IN PROGRESS** | v4 dark-purple 3-column layout live on main (build 2026-06-18.3), tagged pre-v4-redesign for instant revert. Stub tabs remain (Library, Insight, Snapshots, Marketing, Settings). Two reference mockup files added 2026-08-04 — docs/mockups/redesign-v5-mixcheck-dashboard.html and docs/mockups/ozone-redesign-v1.dc.html — **confirmed 2026-08-16 (direct content inspection, Codex-reviewed) to be the same "Ozone 12" graphite/teal-blue MixCheck design in two export/packaging formats, not two competing directions** (same product-level UI — not claiming byte-identical files). Both implement the MixCheck tab only; Library/Insight/Snapshots/Settings appear as inert nav labels with no tab content, Marketing doesn't appear in the tab strip at all. Not yet reviewed/approved by Kevin (interactive GitHub Pages review per docs/CLAUDE.md, not done) — the decision is a single approve/reject of this as the **MixCheck baseline**, not the app's overall skin. Epic not settled — blocks the Hope→Mia rename (see docs/ROADMAP.md). |
