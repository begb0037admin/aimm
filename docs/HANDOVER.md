...
## 2026-09-06 — Hope-rail speaking waveform gain fix ROUND 2 (Markey) — round 1 did NOT work live

**Round 1 (below, build `2026-09-06.3`, merged to `main` @ `20c9384`) did NOT fix the symptom.** Kevin tested it live and reported "no change." Confirmed this was not a deploy/caching issue — the live GitHub Pages site WAS serving `2026-09-06.3`'s actual code (verified directly). The bug was a design-intent flaw, not a code-correctness bug, which is why 5 rounds of Codex TP2 review on round 1 didn't catch it: `feedReal()`'s `realRef` was a same-signal exponential moving average of `rms` lagging by only `REAL_REF_TAU_MS=550`ms. Word/sentence-level loudness changes happen on that timescale or slower, so `gain = REAL_REF_TARGET / realRef` converged such that `rms * gain -> REAL_REF_TARGET` almost immediately regardless of absolute loudness — a textbook AGC/compressor (removes dynamic range), which is the exact opposite of a VU-meter-style visualization's job (reveal dynamic range). The code was doing precisely what a compressor is supposed to do; it was just the wrong tool for this job.

**Round 2 fix:** replaced the single symmetric 550ms tau with an ASYMMETRIC attack/release follower — `realRef` climbs toward a new LOUDER `rms` over `REAL_REF_ATTACK_TAU_MS=1500`ms, and falls toward a new QUIETER `rms` over `REAL_REF_RELEASE_TAU_MS=4000`ms (both far slower than word/phrase-level speech dynamics of ~0.3-1s). Within any single loud or quiet passage the reference does not catch up before the passage ends, so real contrast survives being displayed, while the reference still auto-levels the overall loudness of a call/turn over several seconds (compensating for genuinely different absolute ranges across calls/voices/volumes). Release slower than attack means a loud spike leaves the reference elevated afterward, so a quiet passage right after a loud one reads further suppressed (more contrast) rather than snapping back instantly.

**Hand-verified BEFORE writing code, then Codex-verified at both TP1 (plan) and TP2 (diff, re-run against a standalone node extraction of the actual committed code, not just reasoning):** sequence quiet(rms=0.30, converged)->LOUD(rms=0.75) for 2000ms->quiet(rms=0.30) for 3000ms. LOUD segment reads `0.885` (t=2000ms) rising from `1.000` (t=0-500ms) — i.e. the FULL 2-second LOUD segment stays in the `0.885-1.000` band throughout, not just briefly at the attack transition. The following QUIET segment reads `0.613` (t=0) rising to `0.698` (t=3000ms) — the full 3-second QUIET segment stays in the `0.613-0.698` band. That's a persistent ~0.2-0.4 gap in the displayed `[0,1]` range for the FULL DURATION of each segment — the actual acceptance test for "shows dynamics," not just a transient blip. A short ~400ms loud word inside ongoing quiet still spikes to `1.000` against a `~0.73-0.75` quiet floor immediately after — even brief loud moments register visibly. Codex flagged (accepted as correct behavior, same as real hardware AGC/VU meters): a single loudness level held for 10+ seconds (many multiples of the taus) still eventually converges `realRef` and flattens the display back toward `REAL_REF_TARGET` regardless of absolute `rms` — real speech essentially never holds one exact RMS that long in practice. Also flagged: `REAL_GAIN_MAX=30` makes `REAL_REF_TARGET` structurally unreachable below `rms≈0.0207`, slightly tighter than `REAL_REF_FLOOR`'s own `0.02` — accepted as deliberate noise-floor protection, not an oversight.

**Did NOT ask Kevin for real `getOutputVolume()` console numbers this round** — reasoned through with hand-worked arithmetic (verified against a standalone node re-implementation of the exact committed code, not just algebra on paper) plus Codex TP1/TP2 review, to fix the fundamental self-cancellation flaw before spending a round-trip on calibration data. If this round ALSO reads as "no change" live, the next step should be the `console.log({rms, gain, norm})` behind a dev-flag capture this round intentionally skipped, per the brief's own suggested fallback.

**Shipped:** `index.html` `feedReal()` in `HOPE_WAVE` (~line 14483) — `REAL_REF_TAU_MS` (550, single) replaced with `REAL_REF_ATTACK_TAU_MS` (1500) + `REAL_REF_RELEASE_TAU_MS` (4000); `feedReal()`'s smoothing now branches `rms > realRef ? ATTACK : RELEASE`. `REAL_REF_TARGET`, `REAL_REF_FLOOR`, `REAL_GAIN_MAX`, the `^0.4` display curve, `resetRealPeak()`, and the `isReal` routing are all UNCHANGED from round 1. `AIMM_BUILD` bumped `2026-09-06.3` → `2026-09-06.4`. Pushed to NEW branch `markey-hopewave-gain-tune-2-work` (off `main` @ `20c9384` — round 1's branch name `markey-hopewave-gain-tune` is already merged, not reused), **not merged**. Codex TP1 (plan + worked example) = PASS (arithmetic independently re-verified, two minor notes accepted as intentional — see above). Codex TP2 (diff + re-run worked example against the actual code) = PASS, one comment-only nit fixed (a leftover reference to the old single-tau design in a nearby comment bullet). TP3 (end-to-end) below.

**Still open:** same as round 1 — this is a mechanism-level fix, hand-verified and Codex-verified numerically, not yet live-amplitude-verified against a real ElevenLabs call. Kevin's own live listen is the remaining confirmation step, and this time it's the FIRST thing to check before assuming the design is right — round 1 already burned one "should work" assumption.

## 2026-09-06 — Hope-rail speaking waveform gain fix ROUND 1 (Markey) — SUPERSEDED, did not work live, see ROUND 2 above

Kevin gave live screenshot feedback: the `#hopeWave` speaking meter "does not look like PTT — there is no headroom... looks constant... too large." This is the residual flagged in `docs/STATUS.md`/`docs/ROADMAP.md` Backlog 8 item D since the 2026-09-02 R3 Mix Check round, now live-confirmed exactly as predicted. **This round's fix was later found NOT to work when Kevin live-tested it — see the ROUND 2 entry above for the real root cause and the actual fix.** `docs/STATUS.md`/`DASHBOARD.html`/`docs/ROADMAP.md` item D now correctly say NOT RESOLVED as of round 1.

**Root cause (confirmed by reading the pinned SDK source, not a live call — no browser/mic tool is available to this agent to run one itself):** `HOPE_WAVE.feed()`'s `rms * 18` then `^0.4` curve was ported verbatim from `windows-mac-dictation` PTT's mic-input RMS domain (~0.02–0.1 typical). Hope's rail instead feeds `Conversation.getOutputVolume()`, which `npm pack @elevenlabs/client@0.1.7` + reading `dist/lib.modern.js` shows is `calculateVolume(getOutputByteFrequencyData())` — an `AnalyserNode.getByteFrequencyData()` average across all ~1024 default-fftSize bins (WebAudio defaults: fftSize 2048, smoothingTimeConstant 0.8, minDecibels -100, maxDecibels -30), i.e. a smoothed dB-mapped whole-spectrum power average, not a time-domain RMS. Structurally a different signal than PTT's mic feed — a constant tuned for the latter pins near the ceiling on the former (docs previously said the gain was `×11`; the live code was actually `×18` — that mismatch is also corrected now).

**Design iteration (worth recording — 3 Codex TP2 rounds ruled out an entire approach family):** the first fix attempt was a peak-follower normalizing each real sample against its own recent ceiling. Round 1 found attack-limiting the peak's rise made it permanently trail a sustained level, re-pinning at 1 forever. Round 2 (instant attack, slow 2500ms decay) found a sustained/slowly-varying level still re-clamped near 1 every frame because decay lagged the signal's own variation. Round 3 (faster 220ms decay) found the opposite failure — quiet passages inflating once the peak decayed near its floor — plus the same re-pinning under smoother declines, worked through with actual numbers each round. **Conclusion: no attack/decay tuning fixes a peak-normalize-to-self design; the whole family pins or pumps.** Final shipped design (round 4-5, approved) drops peak-normalization for an AGC/compressor-style reference tracker: a slowly-lagging EMA reference (`REAL_REF_TAU_MS=550`) sets a gain toward a target (`REAL_REF_TARGET=0.62`), so a sample louder/quieter than the recent ~500ms average reads genuinely bigger/smaller instead of always re-normalizing near its own ceiling; true pauses (rms≈0) still read near-zero regardless of gain. Round 4 also caught a real integration bug (a stale `real > 0.004` threshold in `HOPE_VOICE.tick()` routing valid-but-quiet real readings into the synthetic fallback instead of the real AGC path) — fixed by tracking analyser-availability (`realAvailable`) separately from the reading's value.

**Shipped:** `index.html` — `HOPE_WAVE.feedReal()`/`resetRealPeak()` (name kept, body now AGC state) + `HOPE_WAVE.feed(rms, isReal)` branches on `isReal === true`; `HOPE_VOICE.tick()`/`setSpeaking()` updated (explicit `wasSpeaking` 0->1 transition guard, `realAvailable` tracking). Synth/idle fallback paths byte-unchanged. `AIMM_BUILD` bumped `2026-09-06.2` → `2026-09-06.3`. Branch `markey-hopewave-gain-tune` off `main` @ `2c98bfd`, pushed, **not merged**. Codex TP1 (plan) conditional-approved with refinements folded in; TP2 (diff) went 5 rounds as above, final = approved; TP3 (end-to-end) below.

**Still open:** mechanism-level fix with numerically-worked-through AGC behavior (Codex verified the math each round), not a live-amplitude-verified one — no browser/mic/speaker automation tool is available in this environment, so the agent could not run a real ElevenLabs voice call itself. Kevin's own live listen (same harness he used to report the original bug) is the remaining confirmation step, specifically whether `REAL_REF_TARGET`/`REAL_REF_TAU_MS` need adjustment once heard against real speech.
...
- **D** — Hope's speaking-meter amplitude gain (`×11` on the EL `getOutputVolume()` reading) is only structurally verified — needs a **live voice call** to tune; the synthetic envelope is what renders headless. **SUPERSEDED 2026-09-06 — see the "Hope-rail speaking waveform gain fix" entry near the top of this file; the flat gain (actually `×18` live, not `×11`) is replaced with an AGC reference-tracking design. Kevin's live listen remains the confirmation step.**
