...
## 2026-09-06 — Hope-rail speaking waveform gain fix (Markey)

Kevin gave live screenshot feedback: the `#hopeWave` speaking meter "does not look like PTT — there is no headroom... looks constant... too large." This is the residual flagged in `docs/STATUS.md`/`docs/ROADMAP.md` Backlog 8 item D since the 2026-09-02 R3 Mix Check round, now live-confirmed exactly as predicted. Superseding record for that item — see `docs/STATUS.md` for the current full note; item D there and in `DASHBOARD.html`/`docs/ROADMAP.md` are now marked RESOLVED, pointing back here.
