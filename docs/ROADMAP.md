...
## ✅ P0f — Hope dashboard co-review: she reads it AND edits it by voice SHIPPED (2026-06-11, build 2026-06-11.6)

**Kev's report (transcript, 2026-06-11):** Hope opened the dashboard but said "I can't actually read what's displayed in it from my end" — the read-the-screen-together + remove/add-items-by-voice workflow was gone. Restored and extended:

1. **Hope sees the dashboard now.** New `buildDashboardDigest()` builds the exact data the dashboard renders (numbered priority list from docs/ROADMAP.md Planned sections, In-progress lines, Captured-from-voice inbox with ids, recent ships, her own marks). `open_dashboard` returns it as `dashboard_contents`, and a new **`get_dashboard_state`** tool re-reads it mid-conversation. "What's item one?" is now answerable — she reads the data, not the pixels (she never could read pixels; the old "worked" feeling was doc-reading tools).
2. **`manage_roadmap_inbox`** (new tool) — list / remove / complete / dismiss / edit inbox captures by id or fuzzy title. Removals PUT the full array to the Worker (authoritative — merge-by-id would resurrect deletions).
3. **`mark_roadmap_item`** (new tool) — "P-B's finished, take it off": marks a priority item done/removed in a new KV-backed marks store (`/marks` on the Worker, `aimmRoadmapMarks_v1` locally). DASHBOARD.html strikes the matching card through with a "marked by Hope" badge live; the roadmap FILE gets updated by the next Claude session (voice can't write repo files). `status:active` clears a mark.
4. **Live co-review.** Open dashboard views (overlay iframe or tab) repaint the instant Hope captures/edits/marks — `storage`-event listeners + a `refreshDashboardViews()` postMessage ping. `open_dashboard` also fixed: the previous build passed `'noopener'`, which makes `window.open` return null even on success, so the overlay ALWAYS opened (sometimes alongside a real tab). Now uses a named target (`aimmDashboard`) — real handle, one reused tab.
5. **In-app tool registration through the key relay.** Settings → "🔧 Sync Hope's tools" pushes `elevenlabs-client-tools.json` (now 35 tools) to ElevenLabs THROUGH the Worker — no API key touches the browser. Diff-by-name: existing tools PATCHed in place, missing created, then agent `tool_ids` attached. No orphans, safe to re-run, no Publish needed.

**Kev setup required (one-time):** redeploy the Worker (`worker/src/index.js` — adds PATCH to CORS + the `/marks` route), then click Settings → "🔧 Sync Hope's tools" once so the agent learns the three new tools. Until the redeploy: marks are per-browser only and tool sync errors with a redeploy hint; everything else works.
