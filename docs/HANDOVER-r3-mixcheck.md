...
| _post-ship_ `r3-mixcheck-fixes` | POST-SHIP #3 + #hopeWave + rail padding (Markey) | **`AIMM_BUILD 2026-09-02.2`, `index.html` only, on `r3-mixcheck-fixes` (on top of the Cat pass).** (A) #3 Hope-awareness RESTORED: `buildAppKnowledgeDigest` + `RT_INSTRUCTIONS` + the `eq` focus block + `get_context` rewritten for the Mix Check tab / the Fix Queue (retired-surfaces list: no Repair tab / symptom pills / static Reference cards; "never ask where he saw a fix, never name a Session Snapshot / Repair tab / Insight note / past chat"). New `buildMixCheckState()` (→ `get_context.mix_check`) + `buildMixCheckContextBlock()` ("MIX CHECK — LIVE STATE" — the loaded file + `breakdownData()` + the full `mcFixQueue.list()`), fed into `elStart` opening context + `aichatSend` + re-sent via `sendContextualUpdate` on every `aimm:analysis-complete` and Fix Queue `onChange` when a call is live. KB-grounded advice path restored as `MC_FIXMOVES` — one Claude call per analysis (Anthropic proxy) grounded in `buildResearchDigest()` (YouTube KB) + `buildLibraryDigest()`, returns Hope's spoken read + a per-fix `move` that replaces the `MOVE_PENDING` placeholder text via a post-render DOM patch on `#mcActions .mcq-move` (rev-guarded against a re-analysis race — Codex TP2 pass-1 blocker, fixed, pass-2 clean). **Step 7 rewritten**: the auto-posted "HOPE — MIX BREAKDOWN" + "ACTION ITEM" structured cards REMOVED; on analysis-complete Hope now posts ONE real `role:'ai'` `kind:'mc-read'` conversational turn (local natural-language read immediately, upgraded by the KB call; session-only, stripped from persisted history). `window.mcFixQueue` 8-method contract + `aimm:analysis-complete` + item shape UNCHANGED (only consumed). (B) `#hopeWave` restyled to MATCH the LOCKED `#mcWave` — horizontal `--send-blue` (`#2fa1e6→#a557f4`) gradient, bars re-centred (`y = 30 − h/2`), `transform-origin` `bottom → center`; speech animation (`data-speaking` / `--wave-amp` / `hopeWaveIdle↔hopeWaveTalk` / idle-hide) UNCHANGED. Jules design-reviews. (C) Hope-rail breathing room — `.rail-head` padding `16/14/12 → 22/18/16` (both rules), `.rail-body`/`.aichat-compose` padding up, `.oz-rail-name` `line-height 1.05 → 1.22` + `overflow:visible` so "Hope" no longer clips. `#mcWave` untouched. 3-col bottom-align verified after (CDP: `#mcSpecs` = `#mcActions` = `#hopeRail` bottom = 1322). Console clean on load + WAV + analysis. Pushed → `origin/r3-mixcheck-fixes` ONLY. |

| _post-ship_ `r3-mixcheck-fixes` | POST-SHIP #1 corridor swap + MID-band fix + 2 loose ends (Cat) | **`AIMM_BUILD 2026-09-02.2 → 2026-09-02.3`, `index.html` only, on `r3-mixcheck-fixes` (on top of the Markey post-ship row above).** One commit, no new global / function, no DSP or structural change; `#mcWave` + all Markey/Hope-rail code + `corridorAt` / `refPtsFromDbBins` / `ozBandDelta` / the BS.1770-4 engine all untouched. **(a) `REF_CORRIDORS` swap** — the 7 `pts` arrays (`trap` / `hiphop` / `rnb` / `pop` / `afrobeats` / `lofi` / `flat`) replaced verbatim with Jules SPEC v2 §4 (`docs/corridor-retune-spec.md` @ `d1023cb`, Kevin-approved). Keys, `label` strings, the 12 fixed anchor frequencies (20/40/90/160/300/600/1200/2500/5000/10000/16000/20000) and the `[f,lo,hi]` triple shape unchanged. Primary correction: mid/high centre slope steepened from ~−1.5 dB/oct to the published ~−5.6 dB/oct (Elowsson & Friberg 2017 LTAS quadratic over 12,345 BS.1770-4-normalised commercial masters). **(b) MID meter band** — `ozPopulateBands`: `['ozBandMid',250,4000]` → `['ozBandMid',250,3000]` so the MID deviation band matches `ozBandDelta`'s 150–3000 Hz normalisation window (SPEC v2 §7 flag #1, coupled to (a) — otherwise a well-balanced master reads MID ≈ −2…−3 dB against the steeper curve purely from the 3–4 kHz roll-off the norm window doesn't see). **(b-label)** the visible Mid band card sub-label `250 Hz–4 kHz` → `250 Hz–3 kHz` (kept consistent with the new measurement band). **(c) transport `#refFileName` label** — in `window.refLoadFile`, the filename is now extension-stripped before `refSetEl('refFileName',…)` (drop from the last `.`; whole string kept when there's no dot / it's a leading-dot name), matching the `mcSetHeader` #2 treatment. `refFileMeta` still set to `Loading…`. **(d) `.mc-wave` / `.mc-wave-cap` base CSS** — the two unscoped step-5 base rules prefixed with `#eq.oz-mixcheck ` (Codex TP3 non-blocker, Kevin approved). Zero cross-tab bind (`#mcWave` only renders inside `#eq.oz-mixcheck`) and the later `#eq.oz-mixcheck .mc-wave` rules already win the cascade there → no visual change. **Before/after on an identical synthetic trap master (headless CDP render, Target = Trap):** LOW `+15.7 (HOT) → +13.5 (HOT)`; MID `−2.3 (LOW) → −1.8 (LOW)`; HIGH `−9.3 (LOW VS TARGET) → +5.1 (HOT)`. The HIGH shift (+14.4) matches Jules's exact `ozBandDelta` reimplementation (§5.1 predicted +14.5) — the "reads every properly-topped master as dull in the highs" bug (Kevin's `Paypadream$` screenshot: −13.2) is resolved. Fix Queue re-ranks (highs drop #02 → #03). Console clean on load + WAV load in both renders. 3-col bottom-align holds: `#mcSpecs` = `#mcActions` = `#hopeRail` = 1322 (old and new). **Codex TP2** (`codex exec -s read-only -c model_reasoning_effort=low`, BLOCKERS-only, pre-written diff, session `01a06118`): **NO BLOCKERS** (1/4). **Deferred, flagged for Kevin (separate commit):** SPEC v2 §7 flag #2 — `refPopulate()` hard-codes a −8 LUFS / PLR≥7 target regardless of genre; should read `MC_FIXQUEUE.targetLufsFor()`'s existing genre map. Pushed → `origin/r3-mixcheck-fixes` ONLY. |

| _post-ship_ `r3-mixcheck-fixes` | POST-SHIP #1 corridor **v2 → v2.1** — band thickness restored (Cat) | **`AIMM_BUILD 2026-09-02.3 → 2026-09-02.4`, `index.html` only, on `r3-mixcheck-fixes` (on top of the corridor-v2 row above).** One commit, no new global / function, no DSP / structural / CSS / DOM change; `#mcWave` / `corridorAt` / `refPtsFromDbBins` / `ozBandDelta` / `refDrawCanvas` / the BS.1770-4 engine / all Markey/Hope-rail code untouched. **Only the 7 `pts` arrays in `REF_CORRIDORS` (`~15306`) changed** — replaced verbatim with the **v2.1** block from `docs/corridor-retune-spec.md §4` @ `c4f8c8c` (Kevin flagged that v2 had halved the drawn purple corridor band; v2.1 restores the pre-R3 thickness while keeping v2's curve). Keys, `label` strings, the 12 anchor freqs (20/40/90/160/300/600/1200/2500/5000/10000/16000/20000) and the `[f,lo,hi]` triple shape unchanged; the `REF_CORRIDORS` comment block updated (v2→v2.1, `d1023cb`→`c4f8c8c`, + the mechanical-rule note). The MID-band fix + 2 loose ends from the row above are untouched. **v2.1 rule:** every anchor keeps the v2 midpoint `(lo+hi)/2` **exactly**, half-width restored to the pre-R3 (`dbc793d:index.html` ~15160–15168) genre value; where `v2_midpoint − oldHalfWidth < −46` (canvas floor) `lo` clamped at −46, `hi = 2·mid − lo` (midpoint held; band asymmetric there) — 10 anchors: 20 kHz all 7 genres + 16 kHz hiphop + 10/16 kHz lofi. **Numbers do NOT move — verified two ways:** (1) numeric probe — `ozBandDelta` + `corridorAt` copied verbatim from `index.html`, run on the v2 arrays (`HEAD:index.html`) vs v2.1 arrays (working tree), synthetic trap spectrum: LOW/MID/HIGH `ozBandDelta`, `breakdownData().tonalBalanceDeltas` and the Fix Queue `\|delta\|−1.5` band ranking **all identical to 0.0** for trap + all 7 genres; all 84 anchor midpoints byte-identical v2↔v2.1. (2) live headless-Chrome CDP, same synthetic trap WAV via `window.refLoadFile`, Target = Trap, build `2026-09-02.3` vs `.4`: **LOW +11.1 dB · MID −2.0 dB · HIGH −10.7 dB — identical on both**, verdict tags identical, **console clean on both**. Source-confirmed: `ozBandDelta` (`~15722`) / `bandDelta`+`breakdownData` (`~16530`) / Fix Queue `band-*` (`~16410`) consume `corridorAt()` → `(lo+hi)/2` only; only `refDrawCanvas` (`~15393`) reads `lo`/`hi` → the band thickness (the intended change). **3-col align:** `#mcSpecs` bottom == `#hopeRail` bottom (CDP, no WAV → `#mcActions:empty` `display:none` by design); data-only diff, grid geometry unchanged from `2026-09-02.3`. **Codex TP2** (`codex exec --sandbox read-only -c model_reasoning_effort=low`, BLOCKERS-only): **TP2 VERDICT: PASS**, no blockers. **Renders** (coordinator/session scratchpad): `corridor_v2_vs_v21_render.png` (side-by-side composite) + `mcspecs_v2_build3.png` / `mcspecs_v21_build4.png` — review point: purple band back at pre-R3 thickness, curve + numbers unchanged. Pushed → `origin/r3-mixcheck-fixes` ONLY. |
